...
All the code examples and data files discussed in this article are available for download:

<div style="text-align: center; margin: 2em 0;">
  <a 
    href="/download/03-summarization_examples" 
    style="
  ...
  >
    📥 Download Notebook: 03-summarization_examples.ipynb
  </a>
  <a 
    href="/code/building-llm-applications/ch03/Moby-Dick.txt" 
    download
    style="
      display: inline-block;
      padding: 12px 24px;
...
Let's summarize the book Moby Dick using its text file, Moby-Dick.txt, downloaded from the Project Gutenberg site (www.gutenberg.org). Place this file in your working directory, and load the text into a variable:
...
**NOTE:** Keep in mind that running the code on the full Moby Dick text can get expensive. The provided Moby-Dick.txt file is a shorter version, containing only five sections and about 18,000 tokens. Running the code a few times with this version shouldn't cost much. However, if you plan to run many tests, you may want to reduce the file size even more to save money. Each time you execute a chain with an LLMChain block, you'll be charged. If budget allows, you can use the full version of the book, labeled Moby-Dick_ORIGINAL_EXPENSIVE.txt. The entire Moby Dick text has around 300,000 words, or about 350,000 tokens. Using GPT-5, which costs $1.25 per million tokens, processing the full text would cost about $0.37. Running it multiple times will add up. If you switch to GPT-5-nano, which costs $0.05 per million tokens, the cost drops to around $0.015, making it much cheaper.
...
<div style="text-align: center; margin: 2em 0;">
  <a 
    ...
    onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 6px rgba(0,0,0,0.1)';"
  >
    📥 Tải Notebook: 03-summarization_examples.ipynb
  </a>
  <a 
    href="/code/building-llm-applications/ch03/Moby-Dick.txt" 
    download
    ...
    onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 6px rgba(0,0,0,0.1)';"
  >
    📄 Tải Dữ liệu: Moby-Dick.txt
  </a>
</div>

Trước khi bắt đầu xây dựng, chúng ta sẽ xem xét các kỹ thuật tóm tắt khác nhau, mỗi kỹ thuật phù hợp với các tình huống cụ thể, bao gồm tài liệu lớn, tổng hợp nhiều tài liệu, và xử lý dữ liệu có cấu trúc. Bạn đã làm việc với việc tóm tắt tài liệu nhỏ bằng PromptTemplate trong các ví dụ trước đó, vì vậy chúng ta sẽ bỏ qua phần đó và tập trung vào các tình huống phức tạp hơn.
...
Hãy tóm tắt cuốn sách Moby Dick sử dụng file văn bản của nó, Moby-Dick.txt, tải xuống từ trang web Project Gutenberg (www.gutenberg.org). Đặt file này trong thư mục làm việc của bạn và load văn bản vào một biến:
...
Tôi sẽ đề cập chi tiết về các chiến lược chunking—như chunking theo kích thước và cấu trúc nội dung—trong bài sau. Hiện tại, hãy chia văn bản thành các chunks khoảng 3,000 tokens mỗi cái để mô phỏng context window ngắn hơn so với model GPT-5-nano mà chúng ta sẽ sử dụng.
...
Bây giờ bạn đã sẵn sàng thiết lập chain đầu tiên, sẽ chia tài liệu thành các chunks có kích thước được chỉ định. Trong bài này, bạn sẽ học các kiến thức cơ bản về LCEL, với khám phá chi tiết hơn trong bài tiếp theo:

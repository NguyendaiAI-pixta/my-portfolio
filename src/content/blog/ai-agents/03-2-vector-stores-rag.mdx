...
1. Import the `chromadb` module and create an in-memory client. Note the in-memory client loses data when the session ends:
...
2. Create a collection (a "bucket" for documents + embeddings) and insert the Paestum chunks. We include simple `metadatas` and stable `ids` so results are traceable:
...
3. Perform semantic queries. First request the single best match, then ask for the top 3 to inspect relative distances:
...
1. Khởi tạo client ChromaDB (in-memory):
...
2. Tạo collection và chèn nội dung (documents, metadatas, ids):
...
3. Thực hiện truy vấn semantic (yêu cầu 1 kết quả, sau đó 3 kết quả để so sánh khoảng cách):
...
<LangEN>
Let’s implement RAG by building a chatbot that uses the GPT-5-nano model and a vector database [ Part 3.3 Implementing-rag-from-scratch](/blog/ai-agents/03-3-implementing-rag-from-scratch)
</LangEN>

<LangVI>
Hãy triển khai RAG bằng cách xây một chatbot sử dụng mô hình GPT-5-nano và một vector database [Part 3.3 Implementing-rag-from-scratch](/blog/ai-agents/03-3-implementing-rag-from-scratch)
</LangVI>
